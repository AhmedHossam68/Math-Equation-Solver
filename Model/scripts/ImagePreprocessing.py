...
def pad_image(img, output_path, pad_size):
    PAD_TOP, PAD_LEFT, PAD_BOTTOM, PAD_RIGHT = pad_size
    old_im = Image.open(img)
    old_size = (old_im.size[0] + PAD_LEFT + PAD_RIGHT, old_im.size[1] + PAD_TOP + PAD_BOTTOM)
    new_im = Image.new("RGB", old_size, (255, 255, 255))
    new_im.paste(old_im, (PAD_LEFT, PAD_TOP))
    new_im.save(output_path)
    return True
...
def preprocess_images(input_dir, output_dir):
    for filename in os.listdir(input_dir):
        if filename.endswith(".png"):
            ...
            status = crop_image(input_path, output_path, default_size=[600, 60])
            if not status:
                print(f"{filename} is blank, cropped a white image.")

            status = pad_group_image(output_path, output_path, [8, 8, 8, 8], [[240, 100], [320, 80], [400, 80], [400, 100], [480, 80], [480, 100], [560, 80], [560, 100], [640, 80], [640, 100], [720, 80], [720, 100], [720, 120], [720, 200], [800, 100], [800, 320], [1000, 200]])
            if not status:
                print(f"{filename} is larger than the largest provided bucket size, left unchanged.")

            status = downsample_image(output_path, output_path, 2.0)
            print(f"{filename} preprocessed.")

# Example usage
input_dir = "D:\\Ahmed\\AI\\Project\\IM2LATEX(V2)\\formula_images\\formula_images"
output_dir = "D:\\Ahmed\\AI\\Project\\IM2LATEX(V2)\\Preprocessed_Images"
